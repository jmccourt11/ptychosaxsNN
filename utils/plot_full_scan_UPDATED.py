...
class DiffractionAnalyzer:
    def __init__(self, base_path, scan_number, sample_dir, recon_path, dp_size=512, center_offset_y=100, center_offset_x=0, pixel_size=56):
        """
        ...
        Args:
            ...
            pixel_size (float): Size of pixels in nm (default: 56)
        """
    ...
    def perform_deconvolution(self):
        ...
        for dp in self.dps:
            # Preprocess individual patterns
    ...
    def visualize_probe_positions(self, obj, positions, pixel_size=None):
        """
        ...
        Args:
            ...
            pixel_size (float, optional): Size of pixels in nm. If None, uses self.pixel_size.
        """
    ...
    def load_corrected_positions(self, corrected_positions, pixel_size=None):
        """
    ...
    def multiply_probe_with_object(self, obj, positions, pixel_size=None, use_corrected_positions=False):
        """
    ...
    def plot_full_scan(self, use_deconvolved=False, shift_y=0, shift_x=0, scale_factor=2.0, use_corrected_positions=False, highlight_index=None, show_local_fft=False, pixel_size=None):
        """
        ...
        Args:
            ...
            shift_y (float): Vertical shift in projection pixels (pixel_size nm/pixel)
            shift_x (float): Horizontal shift in projection pixels (pixel_size nm/pixel)
        ...
        shift_y_nm = shift_y * pixel_size  # nm
        shift_x_nm = shift_x * pixel_size  # nm
        ...
        if hasattr(self, 'probe'):
            probe_obj_results = self.multiply_probe_with_object(obj, shifted_positions_calc, pixel_size=pixel_size, use_corrected_positions=use_corrected_positions)
        ...
        if highlight_index is not None and show_local_fft:
            ...
            circ = plt.Circle((x_px, y_px), probe_radius_px, fill=False, color='#fcd83f', linestyle='--', linewidth=2)
            ...
            axs[1].imshow(np.abs(obj_roi_vignette), cmap='gray')
            axs[1].set_title('Object with ROI')
            axs[2].imshow(fft_mag, cmap='jet')
            axs[2].set_title('Local FFT (ROI)')
            if deconv_pattern is not None:
                axs[3].imshow(deconv_pattern, cmap='jet', norm=colors.LogNorm())
                axs[3].set_title('Deconvolved Pattern')
            else:
                axs[3].text(0.5, 0.5, 'No deconvolved pattern', ha='center', va='center')
            plt.tight_layout()
            plt.show()
    ...
    def plot_difference_map(self, scan_index, shift_y=0, shift_x=0, use_deconvolved=False, use_corrected_positions=False,scale_factor=2.5):
        ...
        # Convert shifts from pixels to nm
        shift_y_nm = shift_y * 56
        shift_x_nm = shift_x * 56
    ...
    def calculate_probe_radius(self, show_fit=False, pixel_size=None):
        """
    ...
    def plot_scan_overlay(self, use_deconvolved=False, shift_y=0, shift_x=0, scale_factor=2.0, use_corrected_positions=False, highlight_index=None, show_probe_size=True, save_path=None, pixel_size=None):
        """
        Plot full scan of either original or deconvolved patterns using actual position data.
        Places each diffraction pattern at its corresponding position in the grid.
        ...
        for idx, pos in enumerate(shifted_positions_exp):
            ...
            if highlight_index is not None and idx == highlight_index:
                ...
                if show_probe_size and hasattr(self, 'probe'):
                    ...
                    probe_radius_nm = probe_radius_px * pixel_size
...
scan_numbers=get_sample_scans(scan_info, ' ZCB_9_3D_')['scan_number'].values
phi_angles=get_sample_scans(scan_info, ' ZCB_9_3D_')['phi_angle'].values
selected_scans=[{'scan':5065,'angle':31.0,'shift_y':0,'shift_x':0}]
#%%

dps=ptNN_U.load_h5_scan_to_npy(base_directory+'ptycho/', 5065, plot=False, point_data=True)
...
#model_path = Path('/net/micdata/data2/12IDC/ptychosaxs/trained_model/best_model_ZCB_9_32_no_extra_conv_Unet_epoch_50_pearson_loss.pth')
...
analyzer.plot_scan_overlay(highlight_index=666,use_corrected_positions=False,use_deconvolved=False,scale_factor=2,show_probe_size=True,save_path=None)
analyzer.plot_scan_overlay(highlight_index=666,use_corrected_positions=False,use_deconvolved=True,scale_factor=2,show_probe_size=False,save_path=None)
