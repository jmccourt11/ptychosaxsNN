...
class DiffractionAnalyzer:
    ...
    def plot_full_scan(self, use_deconvolved=False, shift_y=0, shift_x=0, scale_factor=2.0, use_corrected_positions=False, highlight_index=None):
        ...
        obj_path = f"{self.base_path}/results/{self.sample_dir}/fly{self.scan_number}/{self.recon_path}/Niter1000.mat"
        obj = sio.loadmat(obj_path)["object_roi"]
...
base_directory='/net/micdata/data2/12IDC/2024_Dec/'
scan_numbers=get_sample_scans(scan_info, ' RC02_R3D_')['scan_number'].values
phi_angles=get_sample_scans(scan_info, ' RC02_R3D_')['phi_angle'].values
scan_numbers=np.arange(884,1045)
selected_scans=[{'scan':887,'angle':1.0,'shift_y':0,'shift_x':0}]
#%%

dps=ptNN_U.load_h5_scan_to_npy(base_directory+'ptycho/', 887, plot=False, point_data=True)
...
obj=tifffile.imread(f"{base_directory}/results/{sample_dir}/fly{selected_scans[0]['scan']}/{recon_path}/O_phase_roi/O_phase_roi_Niter1000.tiff")
plt.imshow(obj,cmap='gray')
plt.show()

obj_path = f"{base_directory}/results/{sample_dir}/fly{selected_scans[0]['scan']}/{recon_path}/Niter1000.mat"
obj = sio.loadmat(obj_path)["object_roi"]
plt.imshow(np.angle(obj),cmap='gray')
plt.show()

probe_positions=sio.loadmat(f"{base_directory}/results/{sample_dir}/fly{selected_scans[0]['scan']}/{recon_path}/Niter1000.mat")["outputs"]["probe_positions"][0][0]
plt.plot(-probe_positions[:,0],probe_positions[:,1],'o')
plt.show()

ob=sio.loadmat(f"{base_directory}/results/{sample_dir}/fly{selected_scans[0]['scan']}/{recon_path}/Niter1000.mat")
pb=ob['probe']
# Only first mode
pb1=pb[:,:,0]
...
#%%
ri=random.randint(0,len(analyzer.preprocessed_patterns))
fig,ax=plt.subplots(1,2,figsize=(10,5))
im1=ax[0].imshow(analyzer.preprocessed_patterns[ri])
plt.colorbar(im1,ax=ax[0])
im2=ax[1].imshow(analyzer.deconvolved_patterns[ri],cmap='magma')
...
analyzer.plot_scan_overlay(highlight_index=36*18-27,use_corrected_positions=False,use_deconvolved=False,scale_factor=1.25,show_probe_size=True,save_path=None)
analyzer.plot_scan_overlay(highlight_index=36*18-27,use_corrected_positions=False,use_deconvolved=True,scale_factor=1.25,show_probe_size=False,save_path=None)



# %%
# After initializing and loading data into analyzer
sample_patterns, sample_indices = analyzer.extract_sample_patterns(threshold=0.7, use_deconvolved=True,show_mask=True)
...
plt.imshow(sample_patterns[10])#, norm=colors.LogNorm())
