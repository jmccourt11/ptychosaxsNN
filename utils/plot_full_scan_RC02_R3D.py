...
class DiffractionAnalyzer:
    ...
    def load_and_crop_data(self):
        ...
        # ]
        self.dps = self.dps[:, offset_y-dpsize//2:offset_y+dpsize//2,
            offset_x-dpsize//2:offset_x+dpsize//2
        ]
    ...
    def perform_deconvolution(self):
        ...
        for dp in self.dps:
            ...
            print('have to change the preprocess function')
    ...
    def load_positions(self):
        ...
        scan_dir = os.path.join('/mnt/micdata2/12IDC/2024_Dec/positions', f'{self.scan_number:03d}')
    ...
    def plot_full_scan(self, use_deconvolved=False, shift_y=0, shift_x=0, scale_factor=2.0, use_corrected_positions=False, highlight_index=None):
        ...
        obj_path = f"/net/micdata/data2/12IDC/2024_Feb/results/RC02_R3D_/fly{self.scan_number}/roi0_Ndp1280/MLc_L1_p10_gInf_Ndp256_mom0.5_pc0_noModelCon_bg0.1_vi_mm//Niter1000.mat"
    ...
    def plot_difference_map(self, scan_index, shift_y=0, shift_x=0, use_deconvolved=False, use_corrected_positions=False,scale_factor=2.5):
        ...
        # Load object
        obj_path = f"/net/micdata/data2/12IDC/2024_Dec/results/RC02_R3D_/fly{self.scan_number}/roi0_Ndp1280/MLc_L1_p10_gInf_Ndp256_mom0.5_pc0_noModelCon_bg0.1_vi_mm/O_phase_roi/O_phase_roi_Niter1000.tiff"
    ...
    def plot_scan_overlay(self, use_deconvolved=False, shift_y=0, shift_x=0, scale_factor=2.0, use_corrected_positions=False, highlight_indices=None, show_probe_size=True, save_path=None):
        ...
        # Load and process the reconstructed object
        obj_path = f"/net/micdata/data2/12IDC/2024_Dec/results/RC02_R3D_/fly{self.scan_number}/roi0_Ndp1280/MLc_L1_p10_gInf_Ndp256_mom0.5_pc0_noModelCon_bg0.1_vi_mm/Niter1000.mat"
        obj = sio.loadmat(obj_path)["object_roi"]
...
base_directory='/scratch/2024_Dec/'
...
# center_offset_y=1123//2-center[0]
# center_offset_x=1267//2-center[1]

center_offset_y=center[0]
center_offset_x=center[1]

model_path = Path('/net/micdata/data2/12IDC/ptychosaxs/models/ZCB_9_3D/best_model_ZCB_9_Unet_epoch_500_pearson_loss.pth')
#%%
dps=ptNN_U.load_h5_scan_to_npy('/net/micdata/data2/12IDC/2024_Dec/ptycho/', 888, plot=False, point_data=True)
plt.imshow(np.sum(dps,axis=0),norm=colors.LogNorm())
plt.show()
#%%
plt.imshow(dps[40][center_offset_y-640:center_offset_y+640,center_offset_x-640:center_offset_x+640],norm=colors.LogNorm())
plt.show()
#%%
scan_number=888

obj_path = f"/net/micdata/data2/12IDC/2024_Dec/results/RC02_R3D_/fly{scan_number}/roi0_Ndp1280/MLc_L1_p10_gInf_Ndp256_mom0.5_pc0_noModelCon_bg0.1_vi_mm/Niter1000.mat"
obj = sio.loadmat(obj_path)["object_roi"]
...
ob=sio.loadmat("/net/micdata/data2/12IDC/2024_Dec/results/RC02_R3D_/fly888/roi0_Ndp1280/MLc_L1_p10_gInf_Ndp256_mom0.5_pc0_noModelCon_bg0.1_vi_mm/Niter1000.mat")
pb=ob['probe']
# Only first mode
pb1=pb[:,:,0]
...
#%%
import h5py
with h5py.File("/net/micdata/data2/12IDC/2024_Dec/results/RC02_R3D_/fly888/data_roi0_Ndp1280_dp.hdf5",'r') as f:
    index = 666
    full_img = np.sum(f['dp'][()],axis=0).copy()
    mask = ~np.load('/home/beams/PTYCHOSAXS/deconvolutionNN/data/mask/mask_sum_RC02_R3D_1280.npy')    
    full_img=np.array(full_img*mask)
    plt.imshow(full_img,norm=colors.LogNorm())
    plt.show()
#%%
...
#%%
fig,ax=plt.subplots(1,1,figsize=(5,5))
ax.imshow(np.sum(analyzer.dps,axis=0),norm=colors.LogNorm())
plt.show()
...
#%%
analyzer.plot_scan_overlay(highlight_indices=[40,320,770],use_corrected_positions=False,use_deconvolved=False,scale_factor=0.35,show_probe_size=False,save_path=f"{sample_dir}scan_overlay_original_{analyzer.scan_number}.pdf")
...
#%%
# Only threshold
patterns, indices = analyzer.extract_sample_patterns(mask_type='threshold', threshold=0.85,show_mask=True,use_deconvolved=False)
np.save(f'/home/beams/PTYCHOSAXS/deconvolutionNN/notebooks/eval/RC02_R3D_sample_scan_{analyzer.scan_number}_indices_threshold.npy',indices)
#%%
# Only variance
patterns, indices = analyzer.extract_sample_patterns(mask_type='variance', variance_window=8, variance_threshold=0.0025,show_mask=True,use_deconvolved=False)
np.save(f'/home/beams/PTYCHOSAXS/deconvolutionNN/notebooks/eval/RC02_R3D_sample_scan_{analyzer.scan_number}_indices_variance.npy',indices)
#%%
# # Both masks
