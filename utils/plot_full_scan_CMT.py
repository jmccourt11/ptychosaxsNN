...
class DiffractionAnalyzer:
    ...
    def load_and_crop_data(self,bin=True):
        ...
        if bin:
            self.dps = np.array([ptNN_U.bin_ndarray(dp,(256,256),operation='mean') for dp in self.dps])
    ...
    def perform_deconvolution(self):
        ...
        mask = np.load('/home/beams/PTYCHOSAXS/deconvolutionNN/data/mask/mask_CMT_3D_1024.npy')
        ...
        for dp in self.dps:
            ...
            dp=dp*(~ptNN_U.create_circular_waxs_mask_min_value(dp,88)[1])
            resultT, sfT, bkgT = ptNN_U.preprocess_CMT(dp,mask,radius=200)
            ...
            self.preprocessed_patterns.append(resultTa.detach().to("cpu").numpy()[0][0])
            self.scale_factors.append(sfT)
            self.backgrounds.append(bkgT)
    ...
    def plot_full_scan(self, use_deconvolved=False, shift_y=0, shift_x=0, scale_factor=2.0, use_corrected_positions=False, highlight_index=None):
        ...
        obj_path = f"/net/micdata/data2/12IDC/2025_Feb/results/CMT3D_/fly{self.scan_number:03d}/roi0_Ndp256/MLc_L1_p10_g400_Ndp128_mom0.5_pc100_noModelCon_bg0.1_vi_mm/MLc_L1_p10_g400_Ndp256_mom0.5_pc400_bg0.1_vp4_vi_mm/Niter1000.mat"
    ...
    def plot_difference_map(self, scan_index, shift_y=0, shift_x=0, use_deconvolved=False, use_corrected_positions=False,scale_factor=2.5):
        ...
        # Load object
        obj_path = f"/net/micdata/data2/12IDC/2025_Feb/results/CMT3D_/fly{self.scan_number:03d}/roi0_Ndp256/MLc_L1_p10_g400_Ndp128_mom0.5_pc100_noModelCon_bg0.1_vi_mm/MLc_L1_p10_g400_Ndp256_mom0.5_pc400_bg0.1_vp4_vi_mm/O_phase_roi/O_phase_roi_Niter1000.tiff"
    ...
    def plot_scan_overlay(self, use_deconvolved=False, shift_y=0, shift_x=0, scale_factor=2.0, use_corrected_positions=False, highlight_index=None, show_probe_size=True, save_path=None):
        ...
        # Load and process the reconstructed object
        obj_path = f"{self.base_path}/results/{self.sample_dir}/fly{self.scan_number:03d}/roi0_Ndp256/{self.recon_path}/Niter1000.mat"
        obj = sio.loadmat(obj_path)["object_roi"]
    ...
    def sum_patterns(self, selected_indices=None):
        """
        Sum raw, preprocessed, and deconvolved patterns for the scan.
        Optionally filter by selected_indices.
        Returns a dict with 'scan_number', 'raw', 'preprocessed', 'deconvolved'.
        """
        if self.dps is None:
            raise ValueError("Raw patterns not loaded. Call load_and_crop_data().")
        if self.deconvolved_patterns is None or self.preprocessed_patterns is None:
            raise ValueError("Deconvolution not performed. Call perform_deconvolution().")

        if selected_indices is not None:
            raw = self.dps[selected_indices]
            pre = self.preprocessed_patterns[selected_indices]
            dec = self.deconvolved_patterns[selected_indices]
        else:
            raw = self.dps
            pre = self.preprocessed_patterns
            dec = self.deconvolved_patterns

        summed_raw = np.sum(raw, axis=0)
        summed_pre = np.sum(pre, axis=0)
        summed_dec = np.sum(dec, axis=0)
        return {
            'scan_number': self.scan_number,
            'raw': summed_raw,
            'preprocessed': summed_pre,
            'deconvolved': summed_dec
        }

    @staticmethod
    def _retry_h5_operation(operation, max_retries=5, initial_delay=1):
        import time
        delay = initial_delay
        last_exception = None
    ...
    def save_to_h5(self, output_file, result):
        """
        Save a single result dict to H5 file with retry/backoff.
        """
        import h5py
        import numpy as np
        def write_h5():
            ...
            for group_name in ['raw', 'preprocessed', 'deconvolved']:
                with h5py.File(output_file, 'a') as f:
                    ...
                    f.flush()
            return True
        success = self._retry_h5_operation(write_h5)
        if not success:
    ...
    @classmethod
    def process_and_save_combined_h5(cls, scan_numbers, base_path, sample_dir, recon_path, dp_size, center_offset_y, center_offset_x, model_path, mask_path, output_file, indices_file=None, delay_between_scans=0, verification_plot_dir=None):
        """
        Process a list of scans, sum and save combined patterns to an H5 file.
        Optionally filter by indices_file and save verification plots.
        """
        import torch
        import numpy as np
        import h5py
        ...
        if indices_file is not None:
            with h5py.File(indices_file, 'r') as f:
                indices_dict = {int(k.split('_')[1]): f[k][:] for k in f.keys()}
        for scan_number in tqdm(scan_numbers, desc="Processing scans"):
            analyzer = cls(
                base_path=base_path,
            ...
            analyzer.load_and_crop_data(bin=False)
            analyzer.model = self.model
            analyzer.perform_deconvolution()
            selected_indices = indices_dict[scan_number] if indices_dict and scan_number in indices_dict else None
            result = analyzer.sum_patterns(selected_indices=selected_indices)
            analyzer.save_to_h5(output_file, result)
            if verification_plot_dir is not None:
                analyzer.save_verification_plot(result, output_dir=verification_plot_dir)
            if delay_between_scans > 0 and scan_number != scan_numbers[-1]:
                print(f"\nWaiting {delay_between_scans} seconds before processing next scan...", flush=True)
                time.sleep(delay_between_scans)
        print("Processing complete. Results saved to", output_file)

    def save_verification_plot(self, results, output_dir='plots'):
        """
        Save verification plot for a scan (summed deconvolved, preprocessed, raw).
        """
        import os
        import numpy as np
        import matplotlib.pyplot as plt
...
scan_numbers=get_sample_scans(scan_info, ' CMT3D')['scan_number'].values
phi_angles=get_sample_scans(scan_info, ' CMT3D')['phi_angle'].values
selected_scans=[{'scan':62,'angle':0.0,'shift_y':0,'shift_x':0}]
#%%

dps=ptNN_U.load_h5_scan_to_npy(base_directory+'ptycho/', 62, plot=False, point_data=True)
...
plt.plot(1024//2, 1024//2, 'rx', markersize=10, markeredgewidth=2, label='Center (128,128)')
plt.legend()
plt.colorbar()
plt.show()
...
#%%
model_path = Path('/net/micdata/data2/12IDC/ptychosaxs/trained_model/best_model_DELETE_WAXS_MASK_Unet_epoch_500_pearson_loss.pth')

recon_path='MLc_L1_p10_g400_Ndp128_mom0.5_pc100_noModelCon_bg0.1_vi_mm/MLc_L1_p10_g400_Ndp256_mom0.5_pc400_bg0.1_vp4_vi_mm/'
obj=tifffile.imread(f"{base_directory}/results/{sample_dir}/fly{selected_scans[0]['scan']:03d}/roi0_Ndp256/{recon_path}/O_phase_roi/O_phase_roi_Niter1000.tiff")
plt.imshow(obj,cmap='gray')
plt.show()

obj_path = f"{base_directory}/results/{sample_dir}/fly{selected_scans[0]['scan']:03d}/roi0_Ndp256/{recon_path}/Niter1000.mat"
obj = sio.loadmat(obj_path)["object_roi"]
plt.imshow(np.angle(obj),cmap='gray')
plt.show()

probe_positions=sio.loadmat(f"{base_directory}/results/{sample_dir}/fly{selected_scans[0]['scan']:03d}/roi0_Ndp256/{recon_path}/Niter1000.mat")["outputs"]["probe_positions"][0][0]
plt.plot(-probe_positions[:,0],probe_positions[:,1],'o')
plt.show()

ob=sio.loadmat(f"{base_directory}/results/{sample_dir}/fly{selected_scans[0]['scan']:03d}/roi0_Ndp256/{recon_path}/Niter1000.mat")
pb=ob['probe']
# Only first mode
pb1=pb[:,:,0,0]
...
analyzer.perform_deconvolution()
plt.imshow(analyzer.dps_sum,norm=colors.LogNorm())
plt.colorbar()
plt.show()
...
analyzer.plot_scan_overlay(highlight_index=670,use_corrected_positions=False,use_deconvolved=False,scale_factor=2.0,show_probe_size=True,save_path=None)
analyzer.plot_scan_overlay(highlight_index=670,use_corrected_positions=False,use_deconvolved=True,scale_factor=2.0,show_probe_size=False,save_path=None)



# %%
# After initializing and loading data into analyzer
sample_patterns, sample_indices = analyzer.extract_sample_patterns(threshold=0.70, use_deconvolved=True,show_mask=True)
sample_patterns_raw, sample_indices_raw = analyzer.extract_sample_patterns(threshold=0.70, use_deconvolved=False,show_mask=True)



#%%
data_total=np.zeros_like(sample_patterns[0])
for i in range(len(sample_patterns)):
    data=10**(analyzer.scale_factors[sample_indices[i]]*sample_patterns[i]+analyzer.backgrounds[sample_indices[i]])
    #data=10**(analyzer.scale_factors[sample_indices[i]]*analyzer.preprocessed_patterns[sample_indices[i]]+analyzer.backgrounds[sample_indices[i]])
    # if i%50==0:
    ...
    data_total+=data
#%%
plt.imshow(data_total)#,norm=colors.PowerNorm(gamma=2.5))
plt.colorbar()
plt.show()
...
# Plot a random pattern and find peaks
ri = random.randint(0, len(sample_patterns)-1)

# Create figure with 4 subplots
fig, (ax1, ax2, ax3, ax4) = plt.subplots(1, 4, figsize=(25, 8))

# Plot deconvolved pattern
deconv_pattern = sample_patterns[ri]
im1 = ax1.imshow(deconv_pattern, cmap='magma')
ax1.set_title(f'Deconvolved Pattern\nIndex {sample_indices[ri]}, ri={ri}')
plt.colorbar(im1, ax=ax1)

# Plot preprocessed pattern
im2 = ax2.imshow(analyzer.preprocessed_patterns[sample_indices[ri]], cmap='viridis')
ax2.set_title('Preprocessed Pattern')
plt.colorbar(im2, ax=ax2)

# Find peaks in deconvolved pattern
center = (deconv_pattern.shape[0]//2, deconv_pattern.shape[1]//2)
y, x = np.indices(deconv_pattern.shape)
r = np.sqrt((x - center[1])**2 + (y - center[0])**2)

# Get peak positions using local maxima in neighborhoods
neighborhood_size = 5
peak_threshold = 0.75#np.mean(deconv_pattern) + 2*np.std(deconv_pattern)

# Create a mask for peaks above threshold
peak_mask = deconv_pattern > peak_threshold

# Find local maxima in neighborhoods
peak_y = []
peak_x = []
for i in range(neighborhood_size, deconv_pattern.shape[0]-neighborhood_size):
    for j in range(neighborhood_size, deconv_pattern.shape[1]-neighborhood_size):
        if peak_mask[i,j]:
            # Check if center pixel is maximum in neighborhood
            neighborhood = deconv_pattern[i-neighborhood_size:i+neighborhood_size+1,
                                       j-neighborhood_size:j+neighborhood_size+1]
            if deconv_pattern[i,j] == np.max(neighborhood):
                peak_y.append(i)
                peak_x.append(j)

peak_y = np.array(peak_y)
peak_x = np.array(peak_x)
peak_r = np.sqrt((peak_x - center[1])**2 + (peak_y - center[0])**2)

# Find rings that intersect most peaks
r_unique = np.unique(np.round(peak_r))
peak_counts = []
for r_val in r_unique:
    count = np.sum(np.abs(peak_r - r_val) < 15)  # Count peaks within 3 pixels of radius
    peak_counts.append(count)

# Get top 3 rings with most peaks
top_3_indices = np.argsort(peak_counts)[-3:]
ring_radii = r_unique[top_3_indices]

# Plot peaks and rings on deconvolved pattern
ax3.imshow(deconv_pattern, cmap='magma')
ax3.plot(peak_x, peak_y, 'b.', markersize=5, label='Peaks')
for radius in ring_radii:
    circle = plt.Circle(center, radius, fill=False, color='cyan', linewidth=.5)
    ax3.add_patch(circle)
ax3.set_title(f'Peaks and Top 3 Rings\nRadii: {ring_radii.astype(int)}')
ax3.legend()

# Plot radial profile with ring positions
r_int = r.astype(int)
radial_profile = np.bincount(r_int.ravel(), deconv_pattern.ravel()) / np.bincount(r_int.ravel())
ax4.plot(range(len(radial_profile)), radial_profile, 'b-', label='Radial Profile')
for radius in ring_radii:
    ax4.axvline(x=radius, color='r', linestyle='--', alpha=0.5)
ax4.set_title('Radial Profile with Ring Positions')
...
indices = DiffractionAnalyzer.process_multiple_scans(
    ...
    center_offset_y=center_offset_y,
    center_offset_x=center_offset_x,
    threshold=0.70,  # Adjust threshold as needed
    output_file='CMT3D_sample_indices.h5',
    show_mask=False  # Set to True if you want to see the mask for each scan
)
#%%
# Later, you can load the indices from the file
loaded_indices = DiffractionAnalyzer.load_sample_indices('CMT3D_sample_indices.h5')
...
recon_path = 'MLc_L1_p10_g400_Ndp128_mom0.5_pc100_noModelCon_bg0.1_vi_mm/MLc_L1_p10_g400_Ndp256_mom0.5_pc400_bg0.1_vp4_vi_mm/'
dp_size = 1024
center = (718, 742)  # Example center
dps_size = 1024      # Example size, adjust as needed

# Calculate offsets (as in your notebook)
center_offset_y = dps_size//2 - center[0]
center_offset_x = dps_size//2 - center[1]

# List of scan numbers to process
scan_numbers = [52, 53, 54]  # Replace with your actual scan numbers

# Model and mask paths
model_path = '/net/micdata/data2/12IDC/ptychosaxs/trained_model/best_model_DELETE_WAXS_MASK_Unet_epoch_500_pearson_loss.pth'
mask_path = '/home/beams/PTYCHOSAXS/deconvolutionNN/data/mask/mask_CMT_3D_1024.npy'

# Output H5 file
output_file = 'combined_diffraction_patterns_CMT3D.h5'

# (Optional) Indices file if you want to filter patterns
indices_file = 'CMT3D_sample_indices.h5'  # Or None

# (Optional) Directory to save verification plots
verification_plot_dir = 'verification_plots'

DiffractionAnalyzer.process_and_save_combined_h5(
    scan_numbers=scan_numbers,
    base_path=base_path,
    sample_dir=sample_dir,
    recon_path=recon_path,
    ...
    indices_file=indices_file,  # or None if not using
    delay_between_scans=10,     # seconds between scans (optional)
    verification_plot_dir=verification_plot_dir  # or None if not saving plots
)
# %%
