...
#%%
lattice=tifffile.imread('/home/beams/PTYCHOSAXS/NN/ptychosaxsNN/diff_sim/lattices/clathrate_II_simulated_256x256x256.tif')
# Create 3D vignette mask
x, y, z = np.meshgrid(np.linspace(-1, 1, lattice.shape[0]),
                     np.linspace(-1, 1, lattice.shape[1]), 
                     np.linspace(-1, 1, lattice.shape[2]))
...
plt.imshow(np.abs(object_img.cpu().numpy()))
plt.show()
